...
​    创建一个空对象
​    将空对象的`__proto__`链接到构造函数的prototype
...
- Array.isArray()
- constructor 例如：`let arr = []; console.log(arr.constructor === Array);`但是通过构造函数判断不推荐，因为有可能被修改
- instanceof,构造函数的 prototype 和 实例的原型链都有可能会改变，所以判断出的结果不一定一成不变
- Object.prototype.toString,例如：`let arr = []; console.log(Object.prototype.toString.call(arr) === "[Object Array]")`
- isPrototypeOf,例如：`let arr = []; console.log(Object.prototype.isPrototypeOf(arr))`
